...
####################################################################
### Changes ########################################################
# Added two player versus game 
# Changed the background color and the snake colors
# reversed the game funtion key, if the snakeeats the fruits the movement function changes
# Fetch border color only once into a register, and do a single store information inside the loop - no extra loads, no subroutine calls.
.data
...
snakeColor2:    .word    0xffff00      #yellow for player 2
headColor:      .word    0x000000
backgroundColor:.word	 0x808080	 # black
borderColor:    .word	 0x00ff00	 # green	
fruitColor: 	.word	 0xff0000	 

#player count $multiplayer variables
player:   .word 0    #this is used to keep the track of the current player 
...
#Game begining message
beginMessage: .asciiz "Enable two player vs mode?"
#versus messages
versus1: .asciiz "Player one Ready"
versus2: .asciiz "Player Two Ready"
playerwinMessage1: .asciiz "Player One with the high score wins"
playerwinMessage2: .asciiz "Player Two Wins the high score wins"
tie: .asciiz "its a tie game!"
...
start:
#start of the program that executes ONLY at the first start of the game 
li $t0, 0x0066cc
sw $t0, snakeColor 
...
DirectionCheck:	
	...
	# reverseMode is stored in $s0
# $a1 contains the input key (ASCII: w = 119, s = 115, a = 97, d = 100)

	beqz $s0, EndReverseBlock      # If not in reverse mode, skip this block

    	li $t1, 119                    # load ascii 'w'
    	beq $a1, $t1, FlipToDown       # if 'w', flip down

    	li $t1, 115                    # load ascii 's'
    	beq $a1, $t1, FlipToUp	       # if 's', flip up

    	li $t1, 97           	       # load ascii 'a'
    	beq $a1, $t1, FlipToRight      # if 'a', flip right

    	li $t1, 100                    # load ascii 'd'
    	beq $a1, $t1, FlipToLeft       # if 'd', flip left

    	j EndReverseBlock

FlipToDown:
    	li $a1, 115		       # load ascii 's'
    	j EndReverseBlock

FlipToUp:
    	li $a1, 119		       # load ascii 'w'
    	j EndReverseBlock

FlipToRight:
    	li $a1, 100			# load ascii 'd'
    	j EndReverseBlock

FlipToLeft:
    	li $a1, 97			# load ascii 'a'
    	j EndReverseBlock
...
AddLength:
	...
	xori $s0, $s0, 1 # flip the number (reversemode) in $s0 from 1 or 0
	
	li $v0, 1
	move $a0, $s0
	syscall        # Debugging to see if it actually is 0 or 1
...
replay: 
	li $v0, 50      #custom syscall for replay prompt
	la $a0, replayMessage  #load address of replay message
	syscall
	
	beqz $a0, start		#if user chooses replay
	li $v0, 10		#exit program
	syscall
	
versusEnd:
	lw $a0, player		#load current player flag into $t0
	bnez $a0, playerTwoEnd #if it's player 2 (non-zero), jump toplayerTwoEnd
	li $v0, 56		#
	la $a0, lostMessage	#load lost message
	lw $a1, score		#load player 1'score
	syscall
	
	sw $a1, playerScore1	#store player 1's score
	
	li $a0, 1
	sw $a0, player		#set current player to 1
	
	j versusMode		# jump to vrsus mode 
	
playerTwoEnd:
li $v0, 56              #service code for custom print syscall
la $a0, lostMessage	#load address of the lost message 
lw $a1, score		#load player 2's score
syscall

sw $a1, playerScore2	#save player 2's score 

lw $t0, playerScore1	#load player 1's score into $to
lw $t1, playerScore2	#load player 2's score into $t1

beq $t0, $t1, tie1	#if scores are equal, it's a tie 

bgt $t0, $t1, playerTwoWin   #if player 2 has a higher score, they wins

li $v0, 56
la $a0, playerwinMessage1
move $a1, $t0
syscall
...
playerTwoWin:
li $v0, 56	#Custom syscall for player win message 
la $a0, playerwinMessage2 #address of message for player 2 wining 
lw $a1, playerScore2     #load player 2's score 
syscall

j replay

tie1:
li $v0, 55    #custom syscall for tie message 
la $a0, tie   #load tie message address 
li $a1, 5	#assume "5" represets a tie state or message 
syscall

j replay	#jump to replay prompt 
